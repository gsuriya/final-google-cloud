...
export default function TryOnPage({ params }: { params: Promise<{ productId: string }> }) {
  ...
  const handleCapture = async (imageBase64: string) => {
    ...
    try {
      ...
      let errorMessage = 'Try-on failed: Unknown error'

      if (err instanceof Error) {
        errorMessage = `Try-on failed: ${err.message}`
      } else if (typeof err === 'string') {
        errorMessage = `Try-on failed: ${err}`
      } else if (err && typeof err === 'object') {
        // Handle API error responses
        errorMessage = `Try-on failed: ${err.error || err.message || JSON.stringify(err)}`
      }
